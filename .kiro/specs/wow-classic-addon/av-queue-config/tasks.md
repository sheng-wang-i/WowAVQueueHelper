...
- [ ] 1. 声明 SavedVariables 并实现设置加载逻辑
  - [x] 1.1 在 AVQueueHelper.toc 中添加 `## SavedVariables: AVQueueHelperDB`
    - 在 .toc 文件的元数据区域添加 SavedVariables 声明
    - _Requirements: 9.1_

  - [x] 1.2 在 AVQueueHelper.lua 中添加 DEFAULTS 表和 LoadSavedSettings 逻辑
    - 在 CONFIG 表之后添加 `DEFAULTS` 表（logLevel = LOG_LEVEL.INFO, keybind = "F12"）
    - 实现初始化逻辑：若 AVQueueHelperDB 为 nil 则创建空表，遍历 DEFAULTS 填充缺失字段
    - 将 AVQueueHelperDB 中的值加载到 CONFIG.LOG_LEVEL 和 CONFIG.KEYBIND
    - _Requirements: 9.2_

  - [x] 1.3 修改 PLAYER_LOGIN 事件处理器，集成设置加载
    - 在现有 PLAYER_LOGIN handler 开头调用设置初始化逻辑（在 faction 检测之前）
    - 使用 `CONFIG.KEYBIND`（来自 SavedVariables）替代硬编码 "F12" 进行初始绑定
...
- [ ] 2. 实现设置面板 Frame 和斜杠命令
  - [x] 2.1 创建设置面板 Frame
    ...
    - 设置面板尺寸、位置（屏幕居中）、标题文本
    - 初始状态隐藏，支持 ESC 关闭（加入 UISpecialFrames）
    - _Requirements: 9.3_
...
- [ ] 3. 实现日志级别下拉菜单
  - [ ] 3.1 在设置面板中添加日志级别下拉菜单
    ...
    - 初始化时根据 CONFIG.LOG_LEVEL 设置当前选中项
...
- [ ] 5. 更新现有代码中所有硬编码 KEYBIND 引用
  - [ ] 5.1 替换现有代码中硬编码的 CONFIG.KEYBIND 使用
    - 检查 ResetState 函数中的 `SetBindingClick(CONFIG.KEYBIND, ...)` 确保使用动态值（已是动态引用则无需修改）
    - 检查所有 `SetBindingClick` 和 `SetBinding` 调用确保使用 CONFIG.KEYBIND
    - 确保 PrintMessage 中引用 CONFIG.KEYBIND 的地方正确反映动态值
    - _Requirements: 9.8_
