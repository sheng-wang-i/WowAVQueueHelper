...
- [x] 1. 声明 SavedVariables 并实现设置加载逻辑
  - [x] 1.1 在 AVQueueHelper.toc 中添加 `## SavedVariables: AVQueueHelperDB` 并列出 ConfigPanel.lua
    - 在 .toc 文件的元数据区域添加 SavedVariables 声明
    - 在文件列表中按顺序列出 `AVQueueHelper.lua` 和 `ConfigPanel.lua`
    - _Requirements: 1_

  - [x] 1.2 在 AVQueueHelper.lua 中添加 DEFAULTS 表、LoadSavedSettings 函数和 AVQueueHelper_Shared 全局表
    ...
    - 通过 `AVQueueHelper_Shared` 全局表暴露 LOG_LEVEL、CONFIG 等共享状态供 ConfigPanel.lua 使用
    - _Requirements: 2_
...
- [x] 3. 实现日志级别下拉菜单
  - [x] 3.1 在 ConfigPanel.lua 中添加日志级别下拉菜单
    - 使用 `UIDropDownMenu_Initialize` + `UIDropDownMenu_CreateInfo` 创建 DEBUG/INFO/WARN/ERROR 四个选项
    - 通过 `AVQueueHelper_Shared.LOG_LEVEL` 和 `AVQueueHelper_Shared.CONFIG` 访问共享状态
  ...
  - [x] 3.2 编写属性测试：日志级别更新一致性
    - **Property 2: 日志级别更新一致性**
    - 对任意有效日志级别选择，验证更改后 CONFIG.LOG_LEVEL 和 AVQueueHelperDB.logLevel 同时等于所选值
    - **Validates: Requirements 5**

- [ ] 4. 实现快捷键绑定输入框
  - [x] 4.1 在 ConfigPanel.lua 中添加快捷键绑定按钮
    - 创建 Button 显示当前绑定按键文本（从 CONFIG.KEYBIND 读取）
    - 点击进入捕获模式（文本变为"Press binding key..."）
    - 添加 `settingsState.capturingKeybind` 状态变量，通过 `AVQueueHelper_Shared.settingsState` 暴露
    - 添加"快捷键绑定"标签文本
    - _Requirements: 6_
