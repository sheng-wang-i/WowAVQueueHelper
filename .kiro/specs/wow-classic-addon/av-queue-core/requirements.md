...
- **Master_Volume**: 游戏主音量，通过 `GetCVar("Sound_MasterVolume")` 获取、`SetCVar("Sound_MasterVolume", value)` 设置，取值范围为 0.0 到 1.0 的浮点数
- **Volume_Boost_Factor**: 告警音量增幅系数（默认 1.5），表示播放告警音时将 Master_Volume 调整为当前值的 150%
...
**用户故事：** 作为玩家，我希望插件在播放战场准备就绪告警音时自动提高音量，并在告警结束后恢复原始音量，以确保我不会错过战场弹出提醒。

#### 验收标准

1. WHEN 进入 READY 状态且即将播放 Alert_Sound, THE Addon SHALL 通过 `GetCVar("Sound_MasterVolume")` 记录当前 Master_Volume 值作为原始音量
2. WHEN 进入 READY 状态且即将播放 Alert_Sound, THE Addon SHALL 通过 `SetCVar("Sound_MasterVolume", value)` 将 Master_Volume 设置为原始音量乘以 Volume_Boost_Factor（150%）
3. IF 计算后的音量值超过 1.0, THEN THE Addon SHALL 将 Master_Volume 设置为 1.0（系统最大值）
4. WHEN 告警音停止播放（玩家按 F12 进入战场、战场确认超时、或流程因任何原因重置）, THE Addon SHALL 通过 `SetCVar("Sound_MasterVolume", value)` 将 Master_Volume 恢复为之前记录的原始音量值
5. THE Addon SHALL 在 addonState 中保存原始音量值，确保在任何停止告警的代码路径中都能正确恢复
